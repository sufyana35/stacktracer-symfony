...
final class Configuration implements ConfigurationInterface
{
    ...
    public function getConfigTreeBuilder(): TreeBuilder
    {
        ...
        $treeBuilder->getRootNode()
            ...
            ->defaultTrue()
            ->info('Capture POST/PUT/PATCH request body (sanitized)')
            ->end()
            ->integerNode('max_body_size')
            ->defaultValue(10240)
            ->min(0)
            ->max(1048576)
            ->info('Maximum request body size to capture in bytes (0 = unlimited, default 10KB)')
            ->end()
            ->booleanNode('capture_files')
            ->defaultTrue()
            ->info('Capture file upload metadata (name, size, type - not content)')
            ->end()
            ...
            ->defaultNull()
            ->info('Environment name (production, staging, dev). Auto-detects from Symfony kernel.environment if null.')
            ->end()
            ->end()
            ->end()
            ->arrayNode('logging')
            ...
            ->scalarPrototype()->end()
            ->defaultValue([])
            ->info('Exception classes to ignore (FQCN). Supports wildcards like "Symfony\Component\HttpKernel\Exception\*"')
            ->end()
            ->arrayNode('ignored_transactions')
            ->scalarPrototype()->end()
            ->defaultValue([])
            ->info('Transaction names/routes to ignore from tracing')
            ->end()
            ->scalarNode('before_send')
            ->defaultNull()
            ->info('Service ID of a callable to filter/modify events before sending. Return null to drop.')
            ->end()
            ->scalarNode('before_send_transaction')
            ->defaultNull()
            ->info('Service ID of a callable to filter/modify transactions before sending. Return null to drop.')
            ->end()
            ...
            ->end()
            ->booleanNode('capture_soft_fails')
            ->defaultTrue()
            ->info('Capture exceptions for messages that will be retried')
            ->end()
            ->booleanNode('isolate_scope_by_message')
            ->defaultTrue()
            ->info('Push/pop scope for each message to isolate breadcrumbs')
            ->end()
            ->end()
            ->end()
            ->arrayNode('cache')
            ...
            ->scalarPrototype()->end()
            ->defaultValue([])
            ->info('Command names to exclude from tracing (e.g., "cache:clear")')
            ->end()
            ->end()
            ->end()
            ->arrayNode('form')
            ...
            ->addDefaultsIfNotSet()
            ->info('Controller execution tracing (optional - can also use manual withSpan())')
            ->children()
            ->booleanNode('enabled')
            ->defaultFalse()
            ->info('Enable automatic controller span tracing. Disable if using manual spans.')
            ->end()
            ->end()
            ->end()
            ->end()
